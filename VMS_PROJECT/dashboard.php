...
//this page's purpose is to handle volunteer dashboard functions
require_once __DIR__ . '/auth.php';

$feedback = '';

// Join project(user clicks "Apply Now" button)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['join_project'])) {
    ...
    $checkStmt = $pdo->prepare('SELECT 1 FROM Applications WHERE volunteer_id = ? AND project_id = ?');
    $checkStmt->execute([$volunteerId, $projectId]);
    
    // If not applied, creates a new application with 'Pending' status (insert )
    if (!$checkStmt->fetch()) {
        $join = $pdo->prepare('INSERT INTO Applications (volunteer_id, project_id, status) VALUES (?, ?, ?)');
        $join->execute([$volunteerId, $projectId, 'Pending']);
        $feedback = 'Thanks for applying! Your application is pending review.';
    } else {
        // Already applied then inform user
        $feedback = 'You have already applied for this project.';
    }
}

//when listing available projects,it checks(exists) if the user has already applied to each project
$eventsStmt = $pdo->prepare(
    'SELECT p.project_id AS id, p.title, p.start_date AS event_date, p.description,
     ...
     FROM Projects p
     WHERE p.status = \'Open\' OR p.status = \'Active\'
     ORDER BY p.start_date ASC'
);

$eventsStmt->execute([$_SESSION['user']['user_id']]);
$events = $eventsStmt->fetchAll();

// Summary counts for dashboard cards(count of projects and user applied)
