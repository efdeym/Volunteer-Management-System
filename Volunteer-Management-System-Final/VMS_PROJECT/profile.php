...
//get the user data
try {
    $stmt_user = $pdo->prepare("SELECT first_name, last_name, phone, password_hash, role_id FROM Users WHERE user_id = ?");
...
<header class="site-hero" style="min-height: auto; padding-bottom: 2rem;">
  <nav class="site-nav">
    ...
    <div class="nav-links" data-nav>
       <a href="about.html" class="pill">Home</a>
       ...
       <?php if ($user['role_id'] == 1):?>
           <a href="volunteer_history.php" class="pill">Volunteer History</a>
       <?php elseif ($user['role_id'] == 2):?>
           <a href="org_dashboard.php" class="pill">Organization Dashboard</a>
       <?php elseif ($user['role_id'] == 3): ?>
           <a href="admin.php" class="pill">Admin Panel</a>
       <?php endif; ?>
       <a href="logout.php" class="pill">Logout</a>
    </div>
  </nav>
    <div>
      <h1>Hi <?= htmlspecialchars($user['first_name']) ?>!</h1>
      <p class="lead">Manage your personal information and security settings.</p>
    </div>
  </div>
</header>
  ...
  <section class="section">
    <div class="auth-card" style="max-width: 600px; margin: 0 auto;">
      ...
      <?php if ($update_message): ?>
        <div class="alert success" style="color: green; margin-bottom: 1rem;"><?= $update_message ?></div>
      <?php endif; ?>
      <?php if ($error_message): ?>
        <div class="alert danger" style="color: red; margin-bottom: 1rem;"><?= $error_message ?></div>
      <?php endif; ?>
      ...
      <form method="post" class="form-grid">
        ...
        <label>
          Phone Number
          <input type="text" name="phone" value="<?= htmlspecialchars($user['phone']) ?>" placeholder="5551234567">
        </label>
        <button class="btn primary" type="submit" name="update_profile">Update Info</button>
      </form>
      ...
      <form method="post" class="form-grid">
        ...
        <button class="btn secondary" type="submit" name="change_password">Change Password</button>
      </form>

    </div>

    <section class="section" style="text-align: center; padding-top: 0;">
      <p style="font-size: 0.9rem; color: #ffffffff;">
          If you would like to become an <strong>Organization Manager</strong>, please send an email to 
          <a href="mailto:volunteerhub@volunteer.com" style="color: var(--primary); font-weight: bold;">volunteerhub@volunteer.com</a>
      </p>
  </section>
</main>
